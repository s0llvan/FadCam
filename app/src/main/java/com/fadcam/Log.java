...
public class Log {
    ...
    {
        File directory = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS + "/FadCam");

        if (!directory.exists()) {
            boolean dirCreated = directory.mkdirs();
            if (dirCreated) {
                android.util.Log.d(TAG, "Directory created: " + directory.getAbsolutePath());
            } else {
                android.util.Log.e(TAG, "Failed to create directory: " + directory.getAbsolutePath());
            }
        }

        logFile = new File(directory, "logs.html");
        try {
            if(!logFile.exists())
            {
    ...
    private static void writeLogToFile(String logMessage) {
        try (FileWriter fileWriter = new FileWriter(logFile, true)) {
            fileWriter.append(logMessage).append("<br>");
        } catch (IOException e) {
